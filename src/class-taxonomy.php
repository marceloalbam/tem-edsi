 ...
 *
 * @link       https://github.com/malba231180/tti-edsi/blob/master/src/class-taxonomy.php
 * @since      1.0.0
 * @package    tti-edsi
...
class Taxonomy {
	...
	public function __construct( $name, $slug, $post_slug, $tag, $user_args = array(), $meta = array(), $template = '' ) {
		...
		$labels = array(
			...
			'all_items'         => __( 'All', 'tti-edsi' ) . " $plural",
			'parent_item'       => __( 'Parent', 'tti-edsi' ) . " $singular",
			'parent_item_colon' => __( 'Parent', 'tti-edsi' ) . " {$singular}:",
			'edit_item'         => __( 'Edit', 'tti-edsi' ) . " $singular",
			'update_item'       => __( 'Update', 'tti-edsi' ) . " $singular",
			'add_new_item'      => __( 'Add New', 'tti-edsi' ) . " $singular",
			/* translators: placeholder is the singular taxonomy name */
			'new_item_name'     => sprintf( esc_html__( 'New %d Name', 'tti-edsi' ), $singular ),
			'menu_name'         => $plural,
		);
		...
		foreach ( $this->meta_boxes as $key => $meta ) {
			...
			?><tr class="form-field term-<?php echo esc_attr( $slug ); ?>-wrap">
				...
				<td>
					...
					?>
					<p class="description"><?php esc_html_e( 'Enter a value for this field', 'tti-edsi' ); ?></p>
				</td>
			</tr>
			<?php
		}
